...
class Plugin implements PluginInterface
{
    ...
    public static function config(Form $form): void
    {
        ...
        $ParseMethod = new Radio('ParseMethod', array(
            'api' => 'API',
            'webpage' => '网页'), 'api', 
            '已看列表解析方式', 'API 解析相对稳定，但是有最多获取最近 25 部的限制。网页解析速度可能较慢，但能获取更多记录。不影响在看列表。');
        $form->addInput($ParseMethod);

        $Limit = new Text('Limit', NULL, '20', _t('已看列表数量限制'), _t('设置获取数量限制，不建议设置得太大，有被 Bangumi 拉黑的风险。<b>仅当通过网页解析时有效</b>。不影响在看列表。'));
        $form->addInput($Limit);
    }
    ...
    public static function header(): void
    {
        ...
        if (!empty($config->bgmst) && in_array('jq', $config->bgmst))
        {
            echo '<script src="';
            Options::alloc()->pluginUrl('/PandaBangumi/js/jq.min.js');
            echo '"></script>';
        }
